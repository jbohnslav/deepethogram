...
def purge_unlabeled_elements_from_records(records: dict) -> dict:
    ...
    for animal, record in records.items():
        ...
        if labelfile is None:
            log.warning(
                "Record {} does not have a labelfile! Please start and finish labeling. ".format(animal)
...
def make_loss_weight(
    class_counts: np.ndarray, num_pos: np.ndarray, num_neg: np.ndarray, weight_exp: float = 1
